namespace Assets.Scripts.Levels.ConvoyChaos
{
   ...
   /// <summary>
   /// A base class for convoy chaos type levels.
   /// </summary>
   /// <seealso cref="Jundroo.SimplePlanes.ModTools.Level" />
   public abstract class ConvoyChaosLevelBase : Level
   {
      /// <summary>
      ...
      /// <summary>
      /// Initializes a new instance of the <see cref="ConvoyChaosLevelBase"/> class.
      /// </summary>
      ...
      /// <summary>
      /// Gets the end zone script.
      /// </summary>
      /// <value>
      /// The end zone script.
      /// </value>
      protected ConvoyChaosEndZoneScript EndZone { get; set; }
      ...
      /// <summary>
      /// Gets the end level dialog message for when the player fails to stop a vehicle from reaching the end zone.
      /// </summary>
      /// <value>
      /// The end level dialog message for when the player fails to stop a vehicle from reaching the end zone.
      /// </value>
      protected virtual string VehicleReachedEndZoneMessage
      {
         get
         {
            return "You failed to destroy an enemy vehicle before it reached the end zone.";
         }
      }
      ...
      protected virtual void Initialize()
      {
         ...
         if (!this.Initialized.HasValue)
         {
            try
            {
               ...
               this.EndZone = obj.GetComponentInChildren<ConvoyChaosEndZoneScript>();
               if (this.EndZone == null)
               {
                  Debug.LogErrorFormat("Unable to get the end zone script from the level game object '{0}'", this._levelGameObjectName);
                  this.Initialized = false;
                  return;
               }
      ...
      protected override void Update()
      {
         ...
         else if (this.EndZone.VehicleHasReachedEndZone)
         {
            this.EndLevel(false, this.VehicleReachedEndZoneMessage, 0);
         }
      }
   }
}